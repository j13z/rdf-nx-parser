...
var parse = (function () {
	...
	return function (type, input, options) {
		...
		if (typeof input === 'string') {
			// Test if the string is commented out
			if (regexLeadingComment.test(input)) {
				return null;
			}
			...
			// Assume an array of token objects was passed.
			tokens = input;
		}
...
function decodeUcharToken(escapeString) {
	...
	//
	// Still has some issues, like resulting in a "wrong" string lengths. See
	...
	if (escapeString[1] === 'u') {
		...
		// Assertion (should not occur, filtered by regex)
		throw 'Invalid escape string: ' + escapeString;
	}
}
...
parser.tokenize = (function () {
	...
	// This thing does most of this module's work. See `regexes.md` for details.
...
parser.parseTriple = function (input, options) {
	...
	return parse('triple', input, options);
};
...
parser.parseQuad = function (input, options) {
	...
	return parse('quad', input, options);
};
